...
public class FileSelector
{
	...
	private static SelectionCriterion _ParseCriterion(string s)
	{
		...
		string[] array = new string[4] { "\\((\\S)", "( $1", "(\\S)\\)", "$1 )" };
		...
		for (int j = 0; j < array2.Length; j++)
		{
			...
			case "size":
			{
				...
				num = (text.ToUpper().EndsWith("K") ? (long.Parse(text.Substring(0, text.Length - 1)) * 1024) : (text.ToUpper().EndsWith("KB") ? (long.Parse(text.Substring(0, text.Length - 2)) * 1024) : (text.ToUpper().EndsWith("M") ? (long.Parse(text.Substring(0, text.Length - 1)) * 1024 * 1024) : (text.ToUpper().EndsWith("MB") ? (long.Parse(text.Substring(0, text.Length - 2)) * 1024 * 1024) : (text.ToUpper().EndsWith("G") ? (long.Parse(text.Substring(0, text.Length - 1)) * 1024 * 1024 * 1024) : ((!text.ToUpper().EndsWith("GB")) ? long.Parse(array2[j + 2]) : (long.Parse(text.Substring(0, text.Length - 2)) * 1024 * 1024 * 1024)))))));
			...
			case "name":
			{
				...
				if (text2.StartsWith("'"))
				{
					...
					if (!text2.EndsWith("'"))
					{
						do
						{
							...
							if (array2.Length <= j + 2)
							{
								throw new ArgumentException(string.Join(" ", array2, num2, array2.Length - num2));
							}
				...
				nameCriterion.MatchingFileSpec = text2;
			...
			case "attributes":
			{
				...
				attributesCriterion.AttributeString = array2[j + 2];
				attributesCriterion.Operator = comparisonOperator;
				selectionCriterion = attributesCriterion;
				j += 2;
				stack.Push(ParseState.CriterionDone);
				break;
			}
