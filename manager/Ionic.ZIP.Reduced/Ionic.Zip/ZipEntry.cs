...
public class ZipEntry
{
	...
	internal int LengthOfCryptoHeaderBytes
	{
		get
		{
			...
			{
				return _KeyStrengthInBits / 8 / 2 + 2;
			}
	...
	internal static ZipEntry ReadDirEntry(ZipFile zf)
	{
		...
		{
			zipEntry._CompressedFileDataSize = zipEntry.CompressedSize - (zipEntry.LengthOfCryptoHeaderBytes + 10);
	...
	private CrcCalculatorStream InternalOpenReader(string password)
	{
		...
		{
			stream = new WinZipAesCipherStream(archiveStream, _aesCrypto, _CompressedFileDataSize, CryptoMode.Decrypt);
		}
	...
	private void InternalExtract(string baseDir, Stream outstream, string password)
	{
		...
		try
		{
			...
			if (num2 != _Crc32 && ((Encryption != EncryptionAlgorithm.WinZipAes128 && Encryption != EncryptionAlgorithm.WinZipAes256) || _WinZipAesMethod != 2))
			{
			...
			if (Encryption == EncryptionAlgorithm.WinZipAes128 || Encryption == EncryptionAlgorithm.WinZipAes256)
			{
				_aesCrypto.ReadAndVerifyMac(ArchiveStream);
			}
			if (OutputFile != null)
			{
				stream.Close();
	...
	private int _ExtractOne(Stream output)
	{
		...
		Stream stream = null;
		stream = ((Encryption == EncryptionAlgorithm.PkzipWeak) ? new ZipCipherStream(archiveStream, _zipCrypto, CryptoMode.Decrypt) : ((Encryption != EncryptionAlgorithm.WinZipAes128 && Encryption != EncryptionAlgorithm.WinZipAes256) ? archiveStream : new WinZipAesCipherStream(archiveStream, _aesCrypto, _CompressedFileDataSize, CryptoMode.Decrypt)));
		...
		using (CrcCalculatorStream crcCalculatorStream = new CrcCalculatorStream(stream2))
		{
			...
			if (Encryption == EncryptionAlgorithm.WinZipAes128 || Encryption == EncryptionAlgorithm.WinZipAes256)
			{
				...
				_aesCrypto.CalculatedMac = winZipAesCipherStream.FinalAuthentication;
			}
		}
		return result;
	}
	...
	{
		if (Encryption != EncryptionAlgorithm.PkzipWeak && Encryption != EncryptionAlgorithm.WinZipAes128 && Encryption != EncryptionAlgorithm.WinZipAes256 && Encryption != 0)
		{
	...
	private void SetupCrypto(string password)
	{
		...
		else if (Encryption == EncryptionAlgorithm.WinZipAes128 || Encryption == EncryptionAlgorithm.WinZipAes256)
		{
			...
			int lengthOfCryptoHeaderBytes = LengthOfCryptoHeaderBytes;
			_zipfile.SeekFromOrigin(FileDataPosition - lengthOfCryptoHeaderBytes);
			_aesCrypto = WinZipAesCrypto.ReadFromStream(password, _KeyStrengthInBits, ArchiveStream);
		}
	}
	...
	private int ProcessExtraFieldWinZipAes(byte[] Buffer, int j, short DataSize, long posn)
	{
		if (_CompressionMethod == 99)
		{
			...
			if (DataSize != 7)
			{
				throw new BadReadException($"  Inconsistent WinZip AES datasize (0x{DataSize:X4}) at position 0x{posn:X16}");
			}
			...
			if (_KeyStrengthInBits < 0)
			{
				throw new BadReadException($"Invalid key strength ({_KeyStrengthInBits})");
			}
			_Encryption = ((_KeyStrengthInBits == 128) ? EncryptionAlgorithm.WinZipAes128 : EncryptionAlgorithm.WinZipAes256);
			j++;
	...
	private void _ConsAndWriteCentralDirectoryEntry(Stream s)
	{
		...
		if (num7 != 0)
		{
			...
			{
				array[num + num4] = _CommentBytes[num4];
			}
			num += num4;
		}
		s.Write(array, 0, num);
	...
	private bool WantReadAgain()
	{
		...
		if (_aesCrypto != null && CompressedSize - _aesCrypto.SizeOfEncryptionMetadata <= UncompressedSize + 16)
		{
			return false;
		}
	...
	private void _WriteFileData(Stream s)
	{
		...
		try
		{
			...
			{
				stream2 = new WinZipAesCipherStream(countingStream, _aesCrypto, CryptoMode.Encrypt);
			}
			Stream stream3 = null;
			bool flag = false;
			if (CompressionMethod == 8 && _zipfile.CompressionLevel != 0)
			{
			...
			stream2.Close();
			_LengthOfTrailer = 0;
			if (stream2 is WinZipAesCipherStream winZipAesCipherStream)
			{
		...
		_EntryHeader[num++] = (byte)((_Crc32 & 0xFF000000u) >> 24);
		...
		else
		{
			...
			_EntryHeader[num++] = (byte)((_CompressedSize & 0xFF000000u) >> 24);
			...
			_EntryHeader[num++] = (byte)((_UncompressedSize & 0xFF000000u) >> 24);
			if (num3 != 0)
			{
				num = 30 + num2;
		...
		if (Encryption == EncryptionAlgorithm.WinZipAes128 || Encryption == EncryptionAlgorithm.WinZipAes256)
		{
			...
			do
			{
				ushort num5 = (ushort)(_EntryHeader[num] + _EntryHeader[num + 1] * 256);
				short num6 = (short)(_EntryHeader[num + 2] + _EntryHeader[num + 3] * 256);
				if (num5 != 39169)
				{
					num += num6 + 4;
					continue;
				...
				num += 9;
				_EntryHeader[num++] = (byte)(_CompressionMethod & 0xFF);
				_EntryHeader[num++] = (byte)(_CompressionMethod & 0xFF00);
			}
			while (num < num3 - 30 - num2);
		}
		if ((_BitField & 8) != 8)
		{
			...
			{
				countingStream2.Adjust(_EntryHeader.Length);
			}
		...
		else
		{
			...
			array2[num++] = (byte)((_CompressedSize & 0xFF000000u) >> 24);
			...
			array2[num++] = (byte)((_UncompressedSize & 0xFF000000u) >> 24);
		}
