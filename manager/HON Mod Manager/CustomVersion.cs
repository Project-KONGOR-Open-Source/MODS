...
internal class CustomVersion : IComparable<CustomVersion>
{
    ...
    public CustomVersion(string version) => VersionString = version;
    ...
    public int? Revis { get => m_revis; set => m_revis = value; }
    ...
    public int CompareTo(CustomVersion other) => CompareToArray(other);

    #endregion

    public static implicit operator Version(CustomVersion cv) =>
        new(cv.Major.GetValueOrDefault(0), cv.Minor.GetValueOrDefault(0),
            cv.Build.GetValueOrDefault(0), cv.Revis.GetValueOrDefault(0));

    public static implicit operator CustomVersion(Version v) => new(v.Major, v.Minor, v.Build, v.Revision);

    public static implicit operator string(CustomVersion cv) => cv.ToString();

    public static implicit operator CustomVersion(string s) => new(s);

    public override string ToString() => VersionString;

    public static bool operator <(CustomVersion cva, CustomVersion cvb) => cva.CompareTo(cvb) < 0;
    ...
    public int CompareToExpanded(CustomVersion other)
    {
        if (Major.HasValue && other.Major.HasValue)
        {
                ...
                if (Minor.HasValue && other.Minor.HasValue)
                {
                    if (Minor == other.Minor)
                    {
                        if (Build.HasValue && other.Build.HasValue)
                        {
                            ...
                            else { return Build.Value.CompareTo(other.Build.Value); }
                        }
                        else
                        {
                            if (Build.HasValue && !other.Build.HasValue)
                    ...
                    else { return Minor.Value.CompareTo(other.Minor.Value); }
                }
                else
                {
                    if (Minor.HasValue && !other.Minor.HasValue)
            ...
            else { return Major.Value.CompareTo(other.Major.Value); }
        }
        else
        {
            if (Major.HasValue && !other.Major.HasValue)
    ...
    public bool IsCompatibleWith(CustomVersion other) => IsCompatibleWithArray(other);
...
internal class CustomRequirement
{
        ...
        else
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    for (int i = 0; i < sa.Length; i++) Range[i] = new CustomVersion(sa[i]);
                }
            }
        }
    }
    ...
    public static implicit operator CustomRequirement(string s) => new(s);

    public static implicit operator string(CustomRequirement cr) => cr.ToString();
