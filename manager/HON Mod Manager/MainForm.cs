...
public partial class MainForm
{
    ...
    private readonly Dictionary<string, string> m_displayNames = new();

    //collection of all mods loaded by UpdateList()
    private readonly List<Modification> m_mods = new();
    ...
    private readonly List<ModUpdater> m_modUpdaters = new();
    ...
    private void LeaveUpdatingMode()
    {
        ...
        if (m_modUpdaters.Count > 0)
        {
            ...
            m_modUpdaters.Clear();
            foreach (Modification tMod in m_mods) tMod.Updater = null;
    ...
    private static readonly Dictionary<string, ZipFile> OpenZIPs = new();
    ...
    {
        foreach (KeyValuePair<string, ZipFile> OpenZIP in OpenZIPs) OpenZIP.Value.Dispose();
        OpenZIPs.Clear();
    }
    ...
    private void MainForm_Load(object sender, EventArgs e)
    {
        ...
        if (string.IsNullOrEmpty(GameHelper.GameDir) && Environment.OSVersion.Platform == PlatformID.Win32NT)
        {
            ...
            if (s == "yes") { RegistryHelper.RegisterFileExtension(); }
            else if (s == "")
            {
                if (
                    MessageBox.Show(
        ...
        else { RegisterhonmodFileExtensionToolStripMenuItem.Enabled = false; }
    }
    ...
    private void MainForm_FormClosing(object sender, FormClosingEventArgs e)
    {
        ...
        if (e.CloseReason == CloseReason.UserClosing && !m_appliedMods.DeepCompareDictionary(m_enabledMods))
        {
            ...
            else if (UserResponse == DialogResult.Cancel) { e.Cancel = true; }
        }
    }
    ...
    {
        if (GameHelper.GameDir == "") { myStatusLabel.Text = "Not attached to a valid HoN install."; }
        else
        {
            if (m_updatingMode)
            {
    ...
    private void myUpdatingTimer_Tick(object sender, EventArgs e)
    {
        ...
        foreach (ModUpdater tModUpdater in m_modUpdaters)
            if (tModUpdater.Status < ModUpdaterStatus.NoUpdateInformation) { FoundBusyUpdate = true; }
            else
            {
        ...
        if (!FoundBusyUpdate) { LeaveUpdatingMode(); }
        else
        {
    ...
    {
        foreach (ModUpdater tModUpdater in m_modUpdaters) tModUpdater.Abort();
    }
    ...
    private void myListView_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            else { lblDescription.LinkArea = new LinkArea(0, 0); }
    ...
    {
        List<ListViewItem> ToDo = new();
        List<ListViewItem> Done = new();
        foreach (ListViewItem Item in myListView.SelectedItems) ToDo.Add(Item);
        bool Change;

        do
        {
            foreach (ListViewItem Item in Done) ToDo.Remove(Item);
            Done.Clear();
    ...
    private void myContextMenu_Opening(object sender, CancelEventArgs e)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                else { CancelUpdateToolStripMenuItem.Visible = false; }
    ...
    private void ApplyModsToolStripMenuItem_Click(object sender, EventArgs e) { ApplyMods(); }
    ...
    private void EnableAllToolStripMenuItem_Click(object sender, EventArgs e) { EnableSelected(); }

    private void DisableAllToolStripMenuItem_Click(object sender, EventArgs e) { DisableSelected(); }
    ...
    private void RefreshModDisplayToolStripMenuItem_Click(object sender, EventArgs e) { UpdateList(); }
    ...
    private void lblDescription_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e) { LaunchWebBrowser(((Modification)myListView.SelectedItems[0].Tag).WebLink); }

    private void ForumThreadToolStripMenuItem_Click(object sender, EventArgs e) { LaunchWebBrowser("https://github.com/Xen0byte/Heroes-Of-Newerth-Mod-Manager"); }
    ...
    private void MainForm_DragDrop(object sender, DragEventArgs e)
    {
        ...
        if (Directory.Exists(tPath))
        {
            ...
            if (tFiles != null)
            {
                foreach (string SourceFile in tFiles) InstallMod(SourceFile);
                UpdateList();
            }
        }
    }
        ...
        if (Directory.Exists(tPath))
            foreach (string tFile in Directory.GetFiles(tPath, "*.honmod"))
                try
                {
                    ...
                    XmlTextReader myXmlReader = new(tStream);
                    ...
                    if (tMod.Icon == null) { tMod.ImageListIdx = -1; }
                    else
                    {
                ...
                if (tMod.Enabled)
                    foreach (KeyValuePair<string, string> tReq in tMod.Requirements)
                    {
                        ...
                        if (!Found)
                        {
                            ...
                            else { myListView.Items[tMod.Index].ImageIndex = 0; }
    ...
    private Modification FindCycle()
    {
        ...
        foreach (Modification tMod in m_mods)
            ...
            {
                //clear marked flags
                foreach (Modification tMod2 in m_mods) tMod2.Marked = false;
    ...
    private bool ApplyMods() => ApplyMods(false, null);

    private bool ApplyMods(string ExportPath) => ApplyMods(false, ExportPath);

    private bool ApplyMods(bool SilentSucces) => ApplyMods(SilentSucces, null);
        ...
        string ExportPath)
    {
        ...
        try
        {
            Dictionary<string, Stream> OutFiles = new();

            //this will hold all files that were already modified
            Dictionary<string, string> FileVersions = new();
                            ...
                            case "editfile":
                                if (!myXmlReader.IsEmptyElement)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        if (tStream != null) { s = Decode(tStream, ref Encoding); }
                                        else
                                        {
                                              ...
                                              myXmlReader.Name == "editfile"))
                                        {
                                            ...
                                            if (myXmlReader.NodeType == XmlNodeType.Element)
                                            {
                                                ...
                                                else if (myXmlReader.IsEmptyElement) { Source = ""; }
                                                else
                                                {
                                                    Source = "";
                                                ...
                                                switch (Operation)
                                                {
                                                    ...
                                                    case "searchup":
                                                        ...
                                                        if (Source != "")
                                                        {
                                                            ...
                                                            else { Pos1 = s.IndexOf(Source, Pos2); }
    ...
    {
        StreamReader myTextReader = new(Data);
